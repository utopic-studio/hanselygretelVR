...
using Utopic.VR.UtopicCardboard;
...
public class CardboardHead : MonoBehaviour {
  ...
  public event HeadUpdatedDelegate OnHeadUpdated;
	...
	// myCodeHere:
	MyRotationController myRotationController;
	MyFPSInputController myFPSInputController;
  ...
  void Awake() {
	...
	void Start () {
		...
		string mymsg = myparent.gameObject.tag == "Player" ? "_" : "Camera should have as parent a Player tagged object";
		mymsg = myparent.GetComponent<MyRotationController> () == null ? mymsg + "   ... parent Player should have MyRotationController component" : mymsg ;
		mymsg = myparent.GetComponent<MyFPSInputController> () == null ? mymsg + "   ... parent Player should have MyFPSInputController component" : mymsg ;
		Debug.LogWarning (mymsg);
		myRotationController = myparent.GetComponent<MyRotationController> ();
		myFPSInputController = myparent.GetComponent<MyFPSInputController> ();
	}
  ...
  private void UpdateHead() {
    ...
    if (trackRotation) {
      ...
      if (target == null) {
		/*Old, original code (working 100%)*/
        //transform.localRotation = rot;
		/* Old code, modified by me a little.. to work with mode ManualTurning*/
				if (myFPSInputController.fpsInputMode == MyFPSInputController.FPSInputMode.HeadTurning) {
					transform.localEulerAngles = new Vector3 (rot.eulerAngles.x, 0f, rot.eulerAngles.z);
					myFPSInputController.transform.gameObject.SendMessage ("setYRotationAmmountQuaternion", rot);
				} else {
					transform.localRotation = rot; //original code
				}

		

		/*This is my temp code: Allows to turn head without holding Alt. But doesn't work in phone*/
		/*float minimumY = -90f;
		float maximumY = 90;
