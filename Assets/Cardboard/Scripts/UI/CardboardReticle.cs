...
public class CardboardReticle : MonoBehaviour, ICardboardGazePointer {
	...
	#region myModifications
	private enum ReticleType {cardboardDefault, onlyOverObject}
	[SerializeField] private ReticleType type;
	#endregion
  ...
  // Current distance of the reticle (in meters).
	private float reticleDistanceInMeters = 0f;//old value was 10.0f;
	#endregion
  ...
  // Minimum distance of the reticle (in meters).
	#region myModifications
  private const float kReticleDistanceMin = 0f; //0.45f was the old value
	#endregion
  ...
  void Start () {
    ...
    materialComp = gameObject.GetComponent<Renderer>().material;
		...
		#region myModifications
		switch (type) {
		case ReticleType.onlyOverObject:
			reticleDistanceInMeters = 0f;
			break;
		case ReticleType.cardboardDefault:
  ...
  public void OnGazeExit(Camera camera, GameObject targetObject) {
    reticleDistanceInMeters = kReticleDistanceMax;
		...
		switch (type) {
		case ReticleType.onlyOverObject:
			Invoke("hide", Time.deltaTime * reticleGrowthSpeed * 1.5f);
			break;
		case ReticleType.cardboardDefault:
			reticleDistanceInMeters = kReticleDistanceMax;
			break;
